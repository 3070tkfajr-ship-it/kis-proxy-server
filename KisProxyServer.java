...
private static final String AZIZ_PROMPT =
    ...
    "모호한 일반론은 금지야. 그리고 당일트레이딩이니 500개의 1분봉보다는 뒤의 30개~50개 사이의 분봉이 중요하다는 점도 잊지 마. 반드시 이 이미지에서 실제로 관찰되는 구체적 근거(예: 몇 번째 캔들에서 몇 선과 몇 선이 semi 교차했는지, 현재가와 VWAP의 대략적 이격 정도, %K/%D의 대략적 위치)를 최소 1개 이상 문장마다 인용해서 말해. 화면에서 확인 안 되는 내용은 추측하지 말고 '이미지에서 명확히 확인 안 됨'이라고 말해.\n\n" +
    "[분석 시점 구분]\n" +
    ...
    " - 과매도(20 이하)에서 골든크로스 직후 바로 재하락해 실패한 흔적이 보리면, 그 실패 이후의 재진입인지 여부를 확인하고 말해. 실패 직후 재진입이면 점수를 낮춰.\n" +
    " - %K와 %D가 둘 다 50 위에 있으면서 롱을 주장하거나, 둘 다 50 아래에 있으면서 숏을 주장하는 경우는 '모멘텀 순방향'으로 보고 가산. 반대로 역행이면 감점.\n\n" +
    "4. 상대 거래량 (가산점, 최대 +8점) — 총점 상한은 여전히 96점\n" +
    " - 최근 20~30개 캔들 평균 거래량 대비, 지금/직전 의미 있는 캔들의 거래량이 대략 몇 배인지 가늠해.\n" +
    " - 진입 방향과 같은 움직임(돌파·반등)에서 2배 이상이면 +5~8점.\n" +
    " - 셋업은 괜찮은데 거래량이 평균 이하(대략 0.7배 미만)면 가산 0점 + 한 줄 평에 '거래량 부족으로 신뢰도 낮음' 명시.\n" +
    " - 이미지에서 거래량 막대가 안 보이면 이 항목은 0점 처리하고 '이미지에서 거래량 확인 안 됨'이라고 적어.\n\n" +
    "[추가로 반드시 서술할 것 (점수에는 포함하지 않음)]\n" +
    "- 손절가 후보는 틱 단위나 0.2% 미만의 터무니없는 초근접으로 잡지 말고, 주가 변동성에 맞춰 **최소 1% ~ 2% 이상의 현실적인 변동폭** 또는 이미지에서 확인되는 의미 있는 스윙 저점/고점을 기준으로 제시해.\n" +
    "- 그 손절폭(최소 1~2% 이상) 대비, 다음 저항/지지(또는 EMA/VWAP 라인)를 목표가로 삼았을 때의 대략적 손익비(예: 약 1.5:1, 2:1)를 추정해서 1줄로 언급. 손익비가 1:1도 안 나오면 '이 자리는 손익비가 불리하다'고 명확히 말해.\n" +
