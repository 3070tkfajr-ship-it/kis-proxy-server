...
public class KisProxyServer {
    ...
    "모호한 일반론은 금지야. 반드시 이 이미지에서 실제로 관찰되는 구체적 근거(예: 몇 번째 캔들에서 몇 선과 몇 선이 교차했는지, 현재가와 VWAP의 대략적 이격 정도, %K/%D의 대략적 위치)를 최소 1개 이상 문장마다 인용해서 말해. 화면에서 확인 안 되는 내용은 추측하지 말고 '이미지에서 명확히 확인 안 됨'이라고 말해.\n\n" +
    "[채점 기준: 총 96점 만점 (100점은 절대 주지 마. 사후 확정 정보가 아닌 실시간 판단은 항상 불확실성이 남아있기 때문)]\n\n" +
    "롱과 숏 점수는 반드시 동일한 분량과 동일한 깊이로 설명해. 한쪽이 유리하다고 판단되더라도, 불리한 쪽의 VWAP·EMA·Stoch 각 항목을 왜 감점했는지 구체적 근거를 최소 1문장씩 반드시 제시해. '불리해서 낮다'는 식의 한 줄 요약만 하고 끝내면 안 된다.또한 스캘핑(단타)영역은 500개의 봉으로 그리되, 그래서 자동 점수/알림을 만들 때도
    “500개로 지표 계산 → 마지막 30~50개 구간의 정렬·Stoch·거래량으로만 진입 점수 이렇게 나누는 게 맞습니다.\n\n" +
    "1. VWAP (최대 35점)\n" +
