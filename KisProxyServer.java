...
```
// 국내(한투 KRX) 분봉 조회 허용 시간대: 08:00~20:00 KST.
// 요일(주말) 제한은 넣지 않는다 — 실제로 주말에도 한투 API가 가장 최근 완료된 거래일(예: 금요일) 데이터를
// 정상적으로 돌려주는 것이 확인되어, 요일로 막을 근거가 없다. 실제로 문제가 확인된 케이스는
// "평일이지만 아직 오늘 장이 시작되지 않은 이른 시간(예: 월요일 07시)" 뿐이었다.
// 국내 상장 ETF는 코스피/코스닥 정규 종목과 매매시간이 완전히 동일(09:00~15:30)하므로 별도 구분이 필요 없다.
private static boolean isWithinAllowedKrDataWindow() {
    java.time.ZonedDateTime nowKst = java.time.ZonedDateTime.now(java.time.ZoneId.of("Asia/Seoul"));
    String hms = nowKst.format(java.time.format.DateTimeFormatter.ofPattern("HHmmss"));
    return hms.compareTo("080000") >= 0 && hms.compareTo("200000") <= 0;
}
```
...
```
        ...
        if ("D".equalsIgnoreCase(tf)) {
            ...
            // 분봉(1m)은 한투 API 특성상 "당일" 데이터만 존재함 (다른 날짜로 대체 조회 불가).
            // 그래서 장 시작 전에는 어차피 유효한 분봉이 없어 가드가 필요하지만,
            // 일봉(D) 조회는 위쪽에서 이미 처리되어 이 가드의 영향을 받지 않는다.
            if (!isWithinAllowedKrDataWindow()) {
                String msg = "{\"output1\":{},\"output2\":[],\"rt_cd\":\"999\","
                        + "\"msg1\":\"분봉 조회 가능 시간이 아닙니다 (08:00~20:00만 지원). 일봉(,D)은 언제든 조회 가능합니다.\"}";
            ...
            java.util.Set<String> seenTimes = new java.util.HashSet<>(); // 중복 캔들(같은 시각) 방어용
            java.time.ZonedDateTime nowKst = java.time.ZonedDateTime.now(java.time.ZoneId.of("Asia/Seoul"));
            String nowHms = nowKst.format(java.time.format.DateTimeFormatter.ofPattern("HHmmss"));
            String marketClose = "153000";
            String targetHour = nowHms.compareTo(marketClose) > 0 ? marketClose : nowHms;
            String prevTargetHour = null; // 루프가 실제로 전진하고 있는지 확인용
            for (int i = 0; i < 17; i++) {
                ...
                for (int j = 0; j < items.length; j++) {
                    ...
                    // 이미 같은 시각(같은 분)의 캔들을 추가한 적이 있으면 건너뛴다 (중복 방지)
                ...
                if (oldestTime.equals(prevTargetHour) || oldestTime.compareTo(targetHour) >= 0) {
                    // 직전 요청과 동일한 시각이 반복되거나, 시간이 전혀 전진하지 않으면
                    // 무한 중복 방지를 위해 즉시 중단한다.
