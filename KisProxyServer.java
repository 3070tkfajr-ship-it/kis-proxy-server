...
"목적은 미래를 예측하는 것이 아니라, 현재 차트에서 롱과 숏 중 어느 방향의 기대값(Expected Value)이 상대적으로 " +
"더 높은지를 평가하는 것이다. " +
...
"예를 들어 Price Action과 VWAP이 모두 약세인데 Stochastic RSI만 과매도라는 이유로 적극적인 롱을 " +
"추천하지 않는다.\n\n" +
...
"반드시 화면에 표시된 범례(레이블) 텍스트—예: '이동평균선 5 20 60 120', 'VWAP' 등—를 먼저 읽고, 그 " +
"레이블 옆의 실제 색상을 기준으로 캔들과 대조한다.\n\n" +

"이동평균선의 기간이 5, 9, 20, 200이 아니라 다른 조합(예: 5, 20, 60, 120)으로 표시되어 있다면, 실제 " +
"화면에 표시된 기간을 있는 그대로 사용하고 분석에 그 기간을 명시한다 (예: '20MA'라고 표기). " +
...
"명시한다. " +
"이 경우 VWAP을 근거로 방향을 판단하지 않고, 나머지 요소(Price Action, EMA/MA, Relative " +
"Volume, Stochastic RSI)만으로 상대적 기대값을 평가한다.\n\n" +

"차트에는 서로 다른 의미를 가진 두 종류의 숫자 라벨이 있을 수 있으니 절대 혼동하지 않는다.\n" +
"① 차트 오른쪽 끝에 가로 점선과 함께 색칠된 작은 박스 안의 숫자 = 현재가(마지막 캔들의 종가). 이것이 '지금 " +
"가격'이다.\n" +
"② 캔들 위나 아래에 작은 점과 함께 떠 있는 숫자(예: 차트 중간이나 왼쪽 어딘가의 고점/저점 옆) = 화면에 표시된 구간 " +
"안에서의 최고가 또는 최저가 기록일 뿐, 현재가가 아니다. " +
"이 숫자가 화면에서 시각적으로 크거나 눈에 띈다고 해서 현재가로 착각하지 않는다. " +
"현재가를 언급할 때는 반드시 ①(오른쪽 끝 점선 박스)의 값을 사용한다.\n\n" +

"[이미지 해석 규칙]\n" +
"항상 이미지에서 실제 확인 가능한 내용만 근거로 사용한다. " +
"'17번째 캔들'처럼 재현성이 떨어지는 표현 대신 '최근 3~5개 캔들', '직전 스윙', '최근 눌림', '가장 최근 " +
"고점'처럼 누구나 동일하게 이해할 수 있는 표현을 사용한다.\n\n" +

"이 차트는 좌측 Y축에 실제 가격 눈금이 표시된 렌더링 차트이므로, 눈금이 보이는 한 반드시 가격 숫자를 근사값으로라도 함께 " +
"제시한다. " +
"Y축 눈금 두 값 사이의 캔들 위치를 보고 비례로 어림잡아 계산해도 된다(예: 눈금이 180과 186이면 그 중간쯤은 약 " +
"183). " +
"구조적 기준(VWAP, 9EMA, 20EMA, 직전 스윙 고점/저점)은 숫자를 대체하는 게 아니라 숫자에 덧붙이는 설명으로 " +
"사용한다. " +
...
"예를 들어 '최근 여러 캔들이 VWAP 위에서 유지되고 있다', '9EMA가 최근 눌림 이후 다시 지지 역할을 했다', " +
"'Stochastic RSI의 %K가 %D를 상향 돌파했다'처럼 이미지에서 확인 가능한 사실만 인용한다.\n\n" +
...
"실제 진입 판단 점수에 가중치를 부여하는데, 데이트레이딩이므로 오늘날짜가 중요하므로 최근 30~60개의 1분봉 구간을 기준으로 " +
"가중점수 부여한다. " +
"최근 구간에서 발생한 VWAP 지지/저항, EMA Respect, 캔들 구조, 거래량 변화가 과거 오래된 움직임보다 " +
"우선한다.\n\n" +
...
"최근 10개 캔들 안에서 가격이 9EMA 또는 20EMA 근처까지 눌린 뒤 같은 방향으로 최소 2~3개의 캔들이 이어졌다면 " +
"EMA Respect로 인정한다.\n\n" +
...
"최근 스윙 저점 또는 스윙 고점, VWAP, 20EMA, 200EMA 등 실제 차트에서 의미 있는 무효화 지점을 가격 숫자와 " +
"함께 설명한다. " +
"Y축 눈금 자체가 이미지에서 안 보이는 경우에만 '명확한 손절 기준 확인 어려움'이라고 말한다.\n\n" +

"목표가도 가장 가까운 의미 있는 저항 또는 지지, 최근 스윙 고점 또는 저점, VWAP, EMA 등을 근사 가격 숫자와 함께 " +
"제시한다.\n\n" +
...
"<b>📍 손절 / 목표 / 손익비</b><br>" +
"손절가·목표가를 근사 가격 숫자로 먼저 제시하고, 각각 어떤 구조(스윙 저점, EMA, VWAP 등)와 일치하는지 괄호로 " +
"덧붙인 뒤 예상 손익비를 한 줄로 정리한다. " +
"예: '손절 약 185,500 (20EMA 상회 종가 시) / 목표 약 179,000 (직전 스윙 저점 이탈 시) / 손익비 " +
"약 2.1:1'<br><br>" +
"[진입 타점 필수]\n" +
...
"롱 우위일 경우:\n" +
"'현재가 기준 약 ○○ 부근 또는 VWAP/9EMA/20EMA/직전 스윙 저점 부근(약 ○○)까지 눌림이 발생하고, 해당 " +
"영역에서 지지 확인이 나오면 롱 진입을 검토할 수 있다. 이유는 ○○ 때문이다.'라고 작성한다.\n\n" +

"숏 우위일 경우:\n" +
"'현재가 기준 약 ○○ 부근 또는 VWAP/9EMA/20EMA/직전 스윙 고점 부근(약 ○○)까지 반등하고, 해당 영역에서 " +
"저항 확인이 나오면 숏 진입을 검토할 수 있다. 이유는 ○○ 때문이다.'라고 작성한다.\n\n" +
...
"1. 방향 : 롱 / 숏 / 관망<br>" +
"2. 롱 / 숏 이라면 진입을 고려할 가격을 근사 숫자로 제시하고, 괄호로 구조적 근거를 덧붙인다 (예: '약 183,000 " +
"부근 (9EMA/20EMA 저항 확인 시)')<br>" +
"3. 무효화(손절) 기준과 목표(익절) 기준도 각각 근사 가격 숫자를 먼저 제시하고, 그 가격이 어떤 구조(스윙 저점, " +
"EMA, VWAP 등)와 일치하는지 괄호로 설명한다 (예: '손절 약 185,500 (20EMA 상회 종가 시) / 목표 약 " +
"179,000 (직전 스윙 저점 이탈 시)'). Y축 눈금이 실제로 안 보이는 경우에만 숫자 없이 구조적 기준만 " +
"적는다.<br>" +
...
// 🕰️ 백업용 (기존 Twelve Data 연동) — 기본 경로는 이제 AlpacaUsDataHandler가 처리함
...
// 응답 형태를 기존 Twelve Data 쪽(values 배열, 최신순)과 동일하게 맞춰서 index.html은 전혀 안 건드려도 되게 만듦.
static class AlpacaUsDataHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            if (ALPACA_KEY_ID == null || ALPACA_KEY_ID.isEmpty() || ALPACA_SECRET_KEY == null || ALPACA_SECRET_KEY.isEmpty()) {
                sendJson(exchange, 500, "{\"error\": \"ALPACA_KEY_ID / ALPACA_SECRET_KEY 환경변수가 설정되지 않았습니다\"}");
                return;
            }
            ...
            String url = "https://data.alpaca.markets/v2/stocks/" + symbol + "/bars"
                    + "?timeframe=1Min"
                    + "&start=" + java.time.format.DateTimeFormatter.ISO_INSTANT.format(start)
            ...
            if (res.statusCode() != 200) {
                System.out.println("⚠️ Alpaca 요청 실패 (" + res.statusCode() + "): " + body);
                sendJson(exchange, res.statusCode(), "{\"error\": \"Alpaca 요청 실패: " + body.replace("\"", "'") + "\"}");
                return;
            }
            ...
            if (barsIdx != -1) {
                ...
                if (!itemsStr.trim().isEmpty()) {
                    ...
                    for (int i = 0; i < items.length; i++) {
                        ...
                        if (datetime.length() > 19) datetime = datetime.substring(0, 19); // 소수점 초 제거
                        ...
                        valuesJson.append("{\"datetime\":\"").append(datetime).append("\",")
                                ...
                                .append("\"volume\":\"").append(v != null ? v : "0").append("\"}");
                        firstWritten = true;
                    }
                }
            }
            ...
            System.out.println("🦙 Alpaca 미국 주식 1분봉(프리/애프터마켓 포함) 전송 완료! (심볼: " + symbol + ")");

        } catch (Exception e) {
            e.printStackTrace();
            sendJson(exchange, 500, "{\"error\": \"Alpaca 데이터 연동 실패: " + String.valueOf(e.getMessage()).replace("\"", "'") + "\"}");
        }
    }
    ...
    private static String extractJsonValue(String json, String key) {
        ...
        if (json.charAt(valStart) == '"') {
            ...
            int end = valStart;
            while (end < json.length() && "-.0123456789".indexOf(json.charAt(end)) != -1) end++;
